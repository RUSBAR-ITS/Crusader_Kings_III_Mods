...
function Read-Catalog([string]$root, [string]$language, [string]$label) {
    ...
    foreach ($file in Get-ChildItem -LiteralPath (Join-Path $root 'localization') -Recurse -File -Filter "*.yml") {
        $relative = $file.FullName.Substring($root.Length + 1).Replace('\','/')
        $bytes = [IO.File]::ReadAllBytes($file.FullName)
        $content = [Text.UTF8Encoding]::new($false,$true).GetString($bytes).TrimStart([char]0xFEFF)
        if ($content -notmatch ('(?m)^\s*l_' + [regex]::Escape($language) + ':\s*$')) { continue }
        $fileLanguageMatches = $file.Name.EndsWith("_l_$language.yml",[StringComparison]::Ordinal)
        if (-not $fileLanguageMatches) { $structure.Add([pscustomobject]@{Catalog=$label;File=$relative;Line=1;Key='';Problem='Filename language suffix differs from header';Text="l_${language}:"}) }
        $count = 0; $n = 0
        foreach ($line in $content -split '\r?\n') {
            ...
            $entry = [pscustomobject]@{Key=$key;File=$relative;Line=$n;Text=$value}
            $allEntries.Add([pscustomobject]@{Catalog=$label;Key=$key;File=$relative;Line=$n;Text=$value})
            if ($map.ContainsKey($key)) {
                $duplicates.Add([pscustomobject]@{Catalog=$label;Key=$key;FirstFile=$map[$key].File;FirstLine=$map[$key].Line;FirstText=$map[$key].Text;File=$relative;Line=$n;Text=$value;Identical=($map[$key].Text -ceq $value)})
            } else { $map.Add($key,$entry) }
        ...
        $inventory.Add([pscustomobject]@{Catalog=$label;File=$relative;Entries=$count;FilenameLanguageMatches=$fileLanguageMatches;BOM=($bytes.Length -ge 3 -and $bytes[0] -eq 239 -and $bytes[1] -eq 187 -and $bytes[2] -eq 191);SHA256=(Get-FileHash -LiteralPath $file.FullName -Algorithm SHA256).Hash})
    }
    return ,$map
}
...
foreach ($key in $en.Keys | Sort-Object -CaseSensitive) {
    ...
    foreach ($kind in @('Expressions','References','Icons','Formatting','Escapes')) {
        ...
        $right=([regex]::Matches([string]$b.Text,$pattern) | ForEach-Object Value | Sort-Object -CaseSensitive) -join ' || '
...
$duplicates | Export-Csv -LiteralPath (Join-Path $out 'duplicates-review.csv') -Encoding UTF8 -NoTypeInformation
$allEntries | Export-Csv -LiteralPath (Join-Path $out 'catalog-entries.csv') -Encoding UTF8 -NoTypeInformation
$stats=[ordered]@{
    MissingFromTranslation=@($en.Keys | Where-Object { -not $ru.ContainsKey($_) }).Count
    MissingFromTranslationAndBuiltin=@($en.Keys | Where-Object { -not $ru.ContainsKey($_) -and -not $builtin.ContainsKey($_) }).Count
    DuplicateRows=$duplicates.Count
    Method="Catalogs selected by language header; filename mismatches reported separately. Declared keys retained on missing closing quote. All physical entries retained; paired rows use first encountered definition, not a claim about engine precedence."; EnglishKeys=$en.Count; TranslationKeys=$ru.Count; BuiltinRussianKeys=$builtin.Count
