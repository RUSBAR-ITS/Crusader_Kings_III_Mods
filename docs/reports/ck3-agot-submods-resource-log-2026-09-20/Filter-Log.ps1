...
function Export-LFCsv {
    param([string]$Name, [Parameter(ValueFromPipeline=$true)]$Row)
    begin { $items = [Collections.Generic.List[object]]::new() }
    process { $items.Add($Row) }
    end {
        [string[]]$lines = @($items | ConvertTo-Csv -NoTypeInformation)
        [RepositoryText]::WriteAllLines((Join-Path $PSScriptRoot $Name), $lines, [Text.UTF8Encoding]::new($true))
    }
}
...
function Resolve-LogFile([string]$File){
    ...
    $value=if(Test-Path -LiteralPath (Join-Path $GamePath $File)){'CK3'}else{'Unresolved file'}
    foreach($mod in $mods){
        foreach($prefix in ($mod.ReplacePaths -split '\|')){
            if($prefix -and $File.StartsWith($prefix.TrimEnd('/')+'/',[StringComparison]::OrdinalIgnoreCase)){$value='Unresolved file'}
        }
        if(Test-Path -LiteralPath (Join-Path $mod.Path $File) -PathType Leaf){$value=$mod.Name}
    }
    $owners[$File]=$value
    return $value
}
...
$entries|Export-LFCsv -Name 'classified-entries.csv'
$duplicateReview|Export-LFCsv -Name 'localization-duplicates.csv'
$allDefs=@(foreach($key in $definitions.Keys){foreach($def in $definitions[$key]){$def}})
$allDefs|Export-LFCsv -Name 'duplicate-definitions.csv'
...
Write-LogEntries @($entries|Where-Object {-not $_.Key -and $_.Category -notin @('Graphics / portrait source error','Save portrait / DNA compatibility')}) 'errors-without-duplicates-or-graphics.log'
...
}|Export-LFCsv -Name 'file-summary.csv'
...
[RepositoryText]::WriteAllText((Join-Path $PSScriptRoot 'owner-summary.json'), ($ownerSummary|ConvertTo-Json -Depth 4), [Text.UTF8Encoding]::new($true))
...
$groups.Values|Sort-Object Count -Descending|Export-LFCsv -Name 'errors-grouped.csv'
...
[RepositoryText]::WriteAllText((Join-Path $PSScriptRoot 'classification-summary.json'), ($summary|ConvertTo-Json -Depth 5), [Text.UTF8Encoding]::new($true))
