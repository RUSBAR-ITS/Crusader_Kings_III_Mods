...
def output(relative, text, owner=None, source=None, reason=''):
    ...
    if relative.endswith('.txt'):
        parse(text)
    bom = INPUTS[owner + '/' + (source or relative)]['bom'] if owner else False
...
''', reason='At most four new languages; cultures sharing a language cannot duplicate it.')
    ...
    output(p, t, 'lotd', reason='Restore Maekar search eligibility. All five missing-CREATOR calls deliberately unchanged.')

    p = 'common/on_action/cowagot_province_on_actions.txt'
    simple('cow', p, [('add_building = common_trade_03','add_building = common_tradeport_03'),
                     ('remove_building = ironwood_01','# USF: absent obsolete building; remove_building = ironwood_01'),
                     ('= bearisland_01','= agot_rodriks_gift_01',2)], 'Known port/Bear Island IDs and obsolete no-op removal only; preserve Harlaw branch.')
    ...
    output(p,t,'cow',reason='Repair scope and avoid downgrade; missing events and religion bonuses deliberately unchanged.')
...
def main():
    ...
    for filename,data in [('source-baseline.json',INPUTS),('source-manifest.json',manifest),('fixes.json',FIXES)]:
