...
def main():
    ...
    runtime=[p.relative_to(b.MOD).as_posix() for root in ['common','events','localization'] for p in (b.MOD/root).rglob('*') if p.is_file()]
    ...
    ok('All LOTD crown calls including CREATOR omissions preserved',func.findall(old)==func.findall(t))
    ...
    ok('Deferred COW events/religion fields untouched',
       re.findall(r'trigger_event\s*=\s*[^\n]+|rhoynish_religion_opinion\s*=\s*\d+',a)==re.findall(r'trigger_event\s*=\s*[^\n]+|rhoynish_religion_opinion\s*=\s*\d+',c))
    ...
    changed=[x['path'] for x in evidence if hashlib.sha256(Path(x['path']).read_bytes()).hexdigest()!=x['sha256']]
    ok('223 inspected existing sources unchanged',not changed)
