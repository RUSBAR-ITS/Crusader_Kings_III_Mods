...
SNAP = HERE / 'snapshot'
PREVIOUS = HERE.parent / 'ck3-agot-plus-fix-stage14-log-2026-09-19'
...
for i, registry in enumerate(load(SNAP/'dlc_load.json')['enabled_mods'],1):
    ...
    mods.append(dict(Order=i,Name=name,Path=path,Descriptor=registry,Exists=True))
export('active-mods.csv',mods)
with (PREVIOUS/'active-mods.csv').open(encoding='utf-8-sig',newline='') as f: old=list(csv.DictReader(f))
assert [(str(m['Order']),m['Descriptor'],m['Path']) for m in mods[:-1]]==[(m['Order'],m['Descriptor'],m['Path']) for m in old]
...
for name,prefix in [('AGOT_PLUS_FIX','plus'),('AGOT_PLUS_RUS_CORRECT','plus-rus'),('AGOT_More_Dragon_Eggs_RUS_CORRECT','mde'),('AGOT_SUBMODS_FIX','submods')]:
    ...
    for rel,digest in files:
        ...
        providers=[Path(m['Path']) for m in mods if (Path(m['Path'])/rel).is_file()]
...
for name in ['expected-log.json','diagnostic-disposition.csv','source-baseline.json','validation.json','installed-validation.json','install-state.json']:
...
save('snapshot-summary.json',dict(CapturedAt=dt.datetime.now().astimezone().isoformat(),Started=launch.isoformat(),ExitLines=[l for l in debug.splitlines() if 'Quit: Quit from inside game' in l],Logs=logs,ActiveMods=len(mods),Previous32ModsUnchanged=True,LauncherAndDlcAgree=True,Mounts=mounts,RuntimeFiles=len(runtime),AllRuntimeHashesMatch=True,AllRuntimeFilesPrecedeLaunch=True,LastDeclaredProvidersMatch=True,EngineIndividualResourceLoadingProven=False,SnapshotNewlines='LF',Copies=copies))
